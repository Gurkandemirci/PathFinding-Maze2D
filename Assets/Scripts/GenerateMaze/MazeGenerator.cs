...
public class MazeGenerator : MonoBehaviour
{
    ...
    public void CreateLayout()
    {
        ...
        CreateCentre();
        RunAlgorithm();
        GameManager.instance.DefineMazeToFirstPanel(mazeParent.transform);

        //MakeExit();
    }

    public void DefineFinishPoint()
    {
        finishRow = Random.Range(1, mazeRows);
        finishColumn = Random.Range(1, mazeColumns);
        Cell finishCell = allCells[new Vector2(finishRow,finishColumn)];
        finishCell.cScript.isFinishPoint = true;
        DefineManhattanDistanceToFinishPoint();
    }

    public void DefineManhattanDistanceToFinishPoint() {
        foreach (KeyValuePair<Vector2, Cell> cell in allCells)
        {
            print(finishRow + " " +  cell.Key.y + " " + finishColumn + " " + cell.Key.x);
            CellScript cellScript = cell.Value.cScript;
            cellScript.manhattanDistance = (int)Mathf.Abs(finishRow - cellScript.coord.x ) + (int)Mathf.Abs(finishColumn - cellScript.coord.y);
        }
    }
    ...
    public void GenerateCell(Vector2 pos, Vector2 keyPos)
    {
        ...
        newCell.cScript.coord = keyPos;
        if (keyPos.x == 1 && keyPos.y == 1)
            newCell.cScript.isStartPoint = true;
    ...
    public void InitValues()
    {
        ...
        mazeParent.AddComponent<MazeManager>();
